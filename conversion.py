...
example_input = '<s>Тема: здраствуйте, давно не слышались? Письмо: ' 
a = '''
tok = GPT2Tokenizer.from_pretrained('models/letters') 
torch_model = GPT2LMHeadModel.from_pretrained('models/letters') 

traced_model = torch.jit.trace(
    torch_model, tok.encode(example_input, return_tensors='pt')
    )
# Convert to Core ML using the Unified Conversion API
model = ct.convert(
    traced_model,
    inputs=[ct.TensorType(tok.encode(example_input, return_tensors='pt'))]
    ) 

model.save("MobileNetV2.mlmodel")
'''

# official hugging faceconverter
...
lm_head_model = GPT2LMHeadModel.from_pretrained('models/letters')
...
model_name = 'letters_exp'
save_spec(builder.spec, f'models/{model_name}-{sequence_length}-{steps}-2.mlmodel')


model = coremltools.models.MLModel(f'models/{model_name}-{sequence_length}-{steps}-2.mlmodel')
